...
export default function AdminCallsPage() {
  ...
  useEffect(() => {
    async function fetchSessions() {
      // Fetch all sessions with student names – no filter on status for now
          ...
          {sessions.map((session) => (
            <GlassCard key={session.id} className="flex justify-between items-center p-4">
              <div>
                ...
                <p className={`text-xs ${session.status === 'scheduled' ? 'text-yellow-600' : session.status === 'active' ? 'text-green-600' : 'text-gray-400'}`}>
                  {session.status}
                </p>
              </div>
              {new Date(session.scheduled_at) > new Date() || session.status === 'active' ? (
